...
with cols[3]:
    st.metric("Runs", kpi_run_count(current_month_df))
with cols[4]:
...
with cols[2]:
    ...
    st.metric("Maximum Quality", f"{quality_max:.1%}" if quality_max == quality_max else "-")
with cols[3]:
    st.metric("Runs", kpi_run_count(rolling_year_df))
with cols[4]:
    st.metric("Average Pace", format_seconds_to_mmss(kpi_average_pace_seconds(rolling_year_df)) + " /km")
with cols[5]:
    st.metric("Total Time", format_seconds_to_hhmmss(kpi_total_time_seconds(rolling_year_df)))
...
with cols[1]:
    ...
    st.metric("Average Quality", f"{quality_avg:.1%}" if quality_avg == quality_avg else "-")
with cols[2]:
    st.metric("Runs", kpi_run_count(df))
with cols[3]:
    st.metric("Total Time", format_seconds_to_hhmmss(kpi_total_time_seconds(df)))
with cols[4]:
