...
with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
    ...
    # # Column widths: size to the longer of the header or a sensible cap,
    # # so long headers/notes don't produce unreadably narrow or wide columns.
    # for i, field in enumerate(FINAL_FIELD_ORDER, start=1):
    #     col_letter = runs_sheet.cell(row=1, column=i).column_letter
    #     max_content_length = df[field].apply(lambda x: len(str(x)) if pd.notna(x) else 0)
    #     width = min(max(len(field), max_content_length) + 2, 40)
